...
// Cek apakah sedang mode Edit (karena admin.php mengirim ?id=...) atau Tambah Baru
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Proses Upload Gambar Poster (Jika ada file yang diupload)
    if (!empty($_FILES['Poster']['name'])) {
        ...
        if (move_uploaded_file($tmp_file, $path_simpan)) {
            // Hapus poster lama dari folder kalau lagi mode edit dan posternya diganti
    ...
    // Eksekusi ke Database (Update atau Insert)
    if ($is_edit) {
...
<head>
    ...
    <style>
        ...
        .form-card { background-color: #121212; border: 1px solid #2a2a2a; border-radius: 12px; }
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                ...
                <div class="d-flex align-items-center mb-4">
                    ...
                    <h3 class="mb-0 text-white fw-light"><?= $is_edit ? 'Edit Data Film' : 'Tambah Jadwal Tayang' ?></h3>
                </div>
                ...
                <div class="form-card p-4 p-md-5 shadow-lg">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <h5 class="text-white mb-4"><i class="bi bi-info-circle text-warning me-2"></i>Informasi Film</h5>
                        
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                <label class="label-gold">Judul Film</label>
                                <input type="text" name="Judul_Film" class="form-control" placeholder="Contoh: One Piece Film: Red" required value="<?= $is_edit ? htmlspecialchars($film['Judul_Film']) : '' ?>">
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="Sutradara" class="form-control" placeholder="Contoh: Goro Taniguchi" required value="<?= $is_edit ? htmlspecialchars($film['Sutradara'] ?? '') : '' ?>">
                            </div>
                            <div class="col-md-6">
                                <label class="label-gold">Studio Produksi</label>
                                <input type="text" name="Studio_Produksi" class="form-control" placeholder="Contoh: Toei Animation" required value="<?= $is_edit ? htmlspecialchars($film['Studio_Produksi'] ?? '') : '' ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="label-gold">Genre</label>
                                <input type="text" name="Genre" class="form-control" placeholder="Action, Adventure..." required value="<?= $is_edit ? htmlspecialchars($film['Genre'] ?? '') : '' ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="label-gold">Durasi (Menit)</label>
                                <input type="number" name="Durasi" class="form-control" placeholder="Contoh: 115" required value="<?= $is_edit ? htmlspecialchars($film['Durasi'] ?? '') : '' ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="label-gold">Rating Usia</label>
                                <select name="Rating_Usia" class="form-select" required>
                                    <option value="SU" <?= ($is_edit && isset($film['Rating_Usia']) && $film['Rating_Usia'] == 'SU') ? 'selected' : '' ?>>SU (Semua Umur)</option>
                                    <option value="13+" <?= ($is_edit && isset($film['Rating_Usia']) && $film['Rating_Usia'] == '13+') ? 'selected' : '' ?>>13+ (Remaja)</option>
                                    <option value="17+" <?= ($is_edit && isset($film['Rating_Usia']) && $film['Rating_Usia'] == '17+') ? 'selected' : '' ?>>17+ (Dewasa)</option>
                                </select>
                            </div>
                            <div class="col-12">
                        ...
                        <h5 class="text-white mb-4 mt-5"><i class="bi bi-camera-reels text-warning me-2"></i>Pengaturan Studio & Harga</h5>
                        ...
                        <div class="row g-3 mb-5">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" name="Kapasitas" class="form-control" placeholder="Maks: 64" min="10" required value="<?= $is_edit ? htmlspecialchars($film['Kapasitas'] ?? '64') : '64' ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="label-gold">Tanggal Tayang</label>
                        ...
                        <div class="d-flex gap-3">
                            <button type="submit" class="btn btn-gold w-100"><i class="bi bi-save me-2"></i> <?= $is_edit ? 'Simpan Perubahan' : 'Simpan Jadwal' ?></button>
